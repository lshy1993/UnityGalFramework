...
namespace Assets.Script.Framework
{
    ...
    public class ImageManager : MonoBehaviour
    {
        ...
        private Image GetSpriteByDepth(int depth)
        {
            Image ui;
            if (depth == -1) return bgSprite;
            if (fgPanel.transform.Find("sprite" + depth) != null)
            {
                ui = fgPanel.transform.Find("sprite" + depth).GetComponent<Image>();
            }
            else
            {
        ...
        /// </summary>
        /// <param name="depth">层编号</param>
        private Image GetTransByDepth(int depth)
        {
            Image ui;
            if (depth == -1) return backTransSprite;
            if (fgPanel.transform.Find("trans" + depth) != null)
            {
                ui = fgPanel.transform.Find("trans" + depth).GetComponent<Image>();
            }
            else
            {
        ...
        {
            GameObject ui;
            if (fgPanel.transform.Find("Live2d_" + depth) != null)
            {
                ui = fgPanel.transform.Find("Live2d_" + depth).gameObject;
            }
            else
            {
                ui = new GameObject();
        ...
        public void RunEffect(NewImageEffect effect, Action callback)
        {
            ...
            switch (effect.operate)
            {
                ...
                case NewImageEffect.OperateMode.Mosaic:
                    GetSpriteByDepth(effect.depth).material = new Material(Shader.Find("Custom/Mosaic"));
                    callback();
                    break;
                case NewImageEffect.OperateMode.Gray:
                    GetSpriteByDepth(effect.depth).material = new Material(Shader.Find("Custom/Gray"));
                    callback();
                    break;
                case NewImageEffect.OperateMode.OldPhoto:
                    GetSpriteByDepth(effect.depth).material = new Material(Shader.Find("Custom/OldPhoto"));
                    callback();
                    break;
        ...
        private void ImageSet(NewImageEffect effect, bool isSprite, bool isAlpha, bool isPos)
        {
            ...
            ui.material = null;
            //ui.material = new Material(Shader.Find("Unity/Transparent Colored"));
            if (isSprite)
            {
        ...
        {
            //设置shader
            Image ui = GetSpriteByDepth(effect.depth);
        ...
        private IEnumerator RotateFade(NewImageEffect effect, Action callback)
        {
            ...
            //ui.material.shader = Shader.Find("Custom/RotateFade");
        ...
        private IEnumerator PreTransByDepth(NewImageEffect effect, Action callback)
        {
            ...
            Image originSprite = GetSpriteByDepth(effect.depth);
            Image transSprite = GetTransByDepth(effect.depth);
            //复制本体给Trans层
        ...
        {
            Image ui = GetSpriteByDepth(effect.depth);
            Image trans = GetTransByDepth(effect.depth);
            //将trans淡出同时淡入原ui
            float t = 0;
        ...
        private IEnumerator Shake(NewImageEffect effect, Action callback)
        {
            ...
            Image sp = GetSpriteByDepth(effect.depth);
